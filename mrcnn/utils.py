...
class Dataset(object):
    ...
    def load_test_json(self, image_id):
         # Read json
        with open('test_img_ids.json') as f:
            data = json.load(f)
        
        image_name, id = data[image_id]['file_name'][:-4], data[image_id]['id']
        return image_name, id
        '''
        for i in range(len(data)):
            if data[i]['file_name'][:-4] == image_name:
                id = data[i]['id']
        '''
    ...
    def load_image_myself(self, image_name):
        """Load the specified image and return a [H,W,3] Numpy array.
        """
        # Load image
        #image = skimage.io.imread(self.image_info[image_id]['path'])
        image = skimage.io.imread('data/test/' + image_name +  '.png')
        # If grayscale. Convert to RGB for consistency.
        if image.ndim != 3:
            image = skimage.color.gray2rgb(image)
        # If has an alpha channel, remove it for consistency
